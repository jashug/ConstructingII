...
\email{jasperh@cs.washington.edu}
...
%The simple elimination rules for the above context-type example begin with motives \[P : \text{Ctx} \to \USet, \qquad Q : (\Gamma : \text{Ctx}) \to \text{Ty}\;\Gamma \to \USet,\] and derive functions \[\text{elim}_P : (\Gamma : \text{Ctx}) \to P\;\Gamma,\qquad \text{elim}_Q : (\Gamma : \text{Ctx})\to (A : \text{Ty}\;\Gamma)\to Q\;\Gamma\;A,\] while the general elimination rules have \[P : \text{Ctx} \to \USet, \qquad Q : (\Gamma : \text{Ctx})\to P\;\Gamma \to \text{Ty}\;\Gamma\to \USet,\] including a dependency on $P\;\Gamma$, and derive functions \[\text{elim}_P : (\Gamma : \text{Ctx}) \to P\;\Gamma,\qquad \text{elim}_Q : (\Gamma : \text{Ctx})\to (A : \text{Ty}\;\Gamma)\to Q\;\Gamma\;(\text{elim}_P\;\Gamma)\;A.\] In \S5.3 \Forsberg/ gives a way to construct these types along with the associated simple elimination rules from indexed inductive types in extensional type theory, and conjectures in \S5.4 that the same approach works in intensional type theory.
