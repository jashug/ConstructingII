...
\begin{enumerate}
    \item $\good{A}\;a$ is a mere proposition only if UIP holds for the type $X$ (Lemma \ref{UIP-from-uniq-good}).
    \item The simple elimination rules from Figure \ref{example-definition} hold for the $(A, B, \eta, \inj, \join)$ constructed above only if $\good{A}\;a$ is a mere proposition (Lemma \ref{uniq-good-from-simple-elim}).
\end{enumerate}
