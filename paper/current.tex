...
\institute{University of Washington, Seattle WA, USA\\
    \email{jasperh@cs.washington.edu} \Letter
    }
...
\begin{figure}[htpb]
    \begin{flushleft}
        Assuming we have $X : \USet$, we consider the inductive-inductive definition\vspace{-.1in}
        \begin{align*}
        A &: \USet,\\
        B &: A \to \USet,\\
        \eta &: X \to A,\\
...
Our goal in this paper is to construct $(A,B,\eta,\join,\inj)$ of the above types given in Figure \ref{example-definition} such that the simple elimination rules (as defined in \citet[\S3.2.5]{nordvallforsberg2013thesis}) hold without using UIP. But first, we will show why \Forsberg/'s approach is not sufficient.
...
UIP (for a type $X$) is the principle that, for all $x : X$, $y : X$, $p : \IdA{x}{y}{X}$, $q : \IdA{x}{y}{X}$, the type $\IdA{p}{q}{\IdA{x}{y}{X}}$ is inhabited. Equivalently, for all $x : X$, $p : \IdA{x}{x}{X}$, the type $\IdA{p}{\refl}{\IdA{x}{x}{X}}$ is inhabited. It expresses that there is at most one proof of any equality. UIP is independent of standard intensional type theory \citep{groupoidmodel}, and is inconsistent with Homotopy Type Theory \citep{hottbook}.

To recap, \citet[\S5.3]{nordvallforsberg2013thesis} constructs an inductive-inductive type by first defining an approximation (the \emph{pre-syntax}) which drops the $A$ index from $B$ to form a mutual inductive definition. Concretely, we have $\pre{A}$ and $\pre{B}$ defined as in Figure \ref{example-pre-syntax}.
...
We will use definition by pattern matching and by induction on the pre-syntax later in our construction.
...
Finally, we define the goodness algebra $\bbO$ by%\vspace{-0.1in}
\begin{gather*}
...
Equivalences between types are very close to equalities between types (the Univalence axiom makes this precise). If we have a \emph{nice} goodness algebra, we have what looks similar to a recursive definition:
\begin{align*}
...
\end{align*}
However, the dependence of $\delta^G.B$ on $\delta^G.A$ makes this what \Forsberg/ calls a `recursive-recursive' definition. It is not immediately clear how to construct a solution to this system; we will expend much effort to do so in \S\ref{ex-limit-alg}. However, once we have a solution, the inductive-inductive object produced by the goodness algebra will satisfy the simple elimination rules.
